...
const AppSidebar: React.FC = () => {
  ...
  useEffect(() => {
    const fetchComplaintCount = async () => {
      try {
        const data = await fetchWithAuth<{ content: any[] }>("http://18.182.12.54:8086/dispute/api/filtered?admin=true&size=100&page=0");
  ...
  const filteredNavItems = userRole === "ADMIN" ? navItems : navItems.filter((item) => item.name !== "👤 User" && item.name !== "🧾Invoice" && item.name !== "💵 Refund");
