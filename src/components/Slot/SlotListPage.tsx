...
const SlotListPage = () => {
  ...
  const fetchLocations = async () => {
    ...
    try {
      ...
      const response = await axios.get("http://18.182.12.54:8082/app-data-service/locations/nearby", {
        params: {
  ...
  const fetchSlots = async (locationId: string) => {
    ...
    try {
      const token = getToken();
      const response = await axios.get("http://18.182.12.54:8082/app-data-service/slots/find-pageable-slots", {
  ...
  const updateSlotStatus = async (slot: any) => {
    ...
    try {
      const token = getToken();
      await axios.put(
        `http://18.182.12.54:8082/app-data-service/slots/status/${slot.id}`,
        {
          status: newStatus,
      ...
      <Dialog open={confirmDialogOpen} onClose={handleCancel}>
        ...
        <DialogActions>
          <Button
            ...
            color="primary"
            sx={{
              backgroundColor: "#4CAF50", // Màu xanh cho Confirm
              color: "#fff", // Màu chữ trắng
              "&:hover": {
                backgroundColor: "#388E3C", // Màu xanh đậm khi hover
              },
            }}
          >
            Confirm
          ...
          <Button
            ...
            sx={{
              backgroundColor: "#BDBDBD", // Màu xám cho Cancel
              color: "#fff", // Màu chữ trắng
              "&:hover": {
                backgroundColor: "#9E9E9E", // Màu xám nhạt khi hover
              },
            }}
          >
            Cancel
