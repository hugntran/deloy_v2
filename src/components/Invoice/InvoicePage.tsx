...
const InvoicePage = ({ searchQuery, locationFilter, typeFilter, fromDate, toDate }: InvoicePageProps) => {
    ...
    try {
      ...
      do {
        const response = await fetchWithAuth<{ content: Location[]; totalElements: number }>(
          `http://18.182.12.54:8082/app-data-service/locations/nearby?longitude=105.779303&latitude=21.028759&maxDistance=100&page=${page}&size=${pageSize}`
        );
  ...
  const fetchInvoices = async (page: number) => {
    try {
      ...
      const response = await fetchWithAuth<{
        ...
        totalPages: number;
      }>(`http://18.182.12.54:8082/app-data-service/api/invoices?${params.toString()}`);
  ...
  const fetchTotalRevenue = async () => {
    ...
    try {
      ...
      >(`http://18.182.12.54:8082/app-data-service/api/invoices/revenue/grouped?${params.toString()}`);
