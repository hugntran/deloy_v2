...
const UserEditPage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`http://18.182.12.54:8080/identity/users/admin/update-user/${userId}`, {
        method: "PUT",
        headers: {
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch("http://18.182.12.54:8085/file/aws/upload-images", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
        },
