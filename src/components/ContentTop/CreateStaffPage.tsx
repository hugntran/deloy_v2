...
export default function CreateStaffPage() {
  ...
  const handleCreate = async () => {
    try {
      ...
      const response = await fetch("http://18.182.12.54:8080/identity/users/create-staff", {
        method: "POST",
        headers: {
  ...
  const handleNextClick = async () => {
    try {
      ...
      const response = await fetch(`http://18.182.12.54:8080/identity/users/get-users-admin?page=${currentPage - 1}&size=${pageSize}`, {
        headers: {
          "Content-Type": "application/json",
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
        },
