...
const AddFacilityForm = () => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await axios.post("http://18.182.12.54:8085/file/aws/upload-images", formData, {
      ...
      // Kiểm tra xem dữ liệu trả về có phải là một mảng và có chứa URL hợp lệ
  ...
  const submitConfirmed = async () => {
    ...
    try {
      ...
      const response = await fetch("http://18.182.12.54:8082/app-data-service/locations/create", {
        method: "POST",
        headers: {
    ...
    <Box sx={{ maxWidth: 600, mx: "auto", p: 2 }}>
      ...
      <Box display="flex" justifyContent="right" mt={3}>
        ...
        <Button
          ...
          sx={{
            marginLeft: "10px",
            backgroundColor: "#d33", // Đặt màu nền
            color: "white", // Đặt màu chữ thành trắng
            "&:hover": {
              backgroundColor: "#c12", // Đổi màu nền khi hover
            },
          }}
        >
          Back
