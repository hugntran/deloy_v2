...
export default function SignInForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("http://18.182.12.54:8080/identity/auth/token", {
        method: "POST",
      ...
      // Giải mã token để lấy userRole từ 'scope'
      const decoded: DecodedToken = jwtDecode(token);
      const userRole = decoded.scope?.replace("ROLE_", "") || "USER"; // Lấy role từ 'scope'
      ...
      // Kiểm tra userRole và điều hướng
      if (userRole !== "ADMIN" && userRole !== "STAFF") {
        // Điều hướng tới trang không hợp lệ hoặc trang 404 nếu không phải ADMIN hoặc STAFF
        navigate("*"); // Điều hướng đến path '*' (trang lỗi 404)
      } else {
        navigate("/home"); // Điều hướng đến trang chính cho ADMIN và STAFF
      }
    } catch (err) {
