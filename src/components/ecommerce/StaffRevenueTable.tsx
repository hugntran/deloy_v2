...
export default function StaffRevenueTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // 1. Fetch all users
        const userRes = await fetch(`http://18.182.12.54:8080/identity/users/get-users-admin?page=0&size=100`, {
          headers: {
        ...
        for (const staff of staffUsers) {
          ...
          const revenueRes = await fetch(`http://18.182.12.54:8082/app-data-service/api/invoices/revenue/grouped?${params.toString()}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
    ...
    fetchData();
  }, [today]); // chỉ cần chạy 1 lần theo ngày hôm nay
